...
import { CalendarIcon, Filter, RefreshCw, Download, Search, X } from "lucide-react";
...
export default function AdvancedFilterPanel({ 
  ...
  const datePresets = [
    { label: "Last 24 hours", value: () => ({ from: subDays(new Date(), 1), to: new Date(), preset: "24h" }) },
    { label: "Last 7 days", value: () => ({ from: subDays(new Date(), 7), to: new Date(), preset: "7d" }) },
    { label: "Last 30 days", value: () => ({ from: subDays(new Date(), 30), to: new Date(), preset: "30d" }) },
    { label: "Last 3 months", value: () => ({ from: subMonths(new Date(), 3), to: new Date(), preset: "3m" }) },
  ];
            ...
            <div className="space-y-3">
              ...
              {filters.dateRange.from && (
                <p className="text-sm text-muted-foreground">
                  ...
                  {filters.dateRange.to ? format(filters.dateRange.to, "MMM dd, yyyy") : "Present"}
                </p>
              )}
            </div>
